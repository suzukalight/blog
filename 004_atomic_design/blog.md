...
昨今、「Atomic Designを取り入れた」という記事も多く見かけるようになりました。弊社でもフロントエンド開発に様々な問題を抱えており、それを解消する目的でAtomic Designを取り入れましたので、それの振り返り。
...
- 特定の機能を充足することに目が行き過ぎ
    ...
    - つまり、コンポーネントは、特定のビューに密結合しており、流用することができていない。
- コンポーネントのテストが存在しない
    - そもそもテスタビリティを考慮していない
    - テストの習慣がないので、テスタビリティのないロジックが散見
    - すぐデグレる（特にスタイル）
    - つまり、そもそもテストを受け入れられない

あるあるかもしれませんが、非常にお恥ずかしい…。完璧より完成を目指していたこともありますが、節操なさすぎでした。

ありがたいことにプロジェクト規模も拡大しているさなか、実装担当者も増えてきたのですが、設計指針となるドキュメントがない。みんなが好き勝手に作り出す前に、指針をつくるべき…。そんな思いから、
...
こういう施策は小さく始めることが肝要だと思っており、まずはエンジニア向けのコンポーネントガイドラインとして利用することとしました。デザイナを巻き込むのは次の段階でチャレンジ。

現状の問題点を踏まえて、以下の問題を解消できることを目的としました。

1. 責任分界点を明確化する
3. 流用可能性を上げること
2. コンポーネントテスタビリティを向上させること


## 責任分界点

Atomic Designには5つのレイヤが存在しますが、フロントエンド実装者としての関心事から考えたとき、状態管理の基準が、レイヤー分けの基準になった
